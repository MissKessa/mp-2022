...
public class Board {
	...
	private void calculateNumberOfMines(int width, int height, int percentage) {
		ArgumentChecks.isTrue(width >= MINIMUM_WIDTH, ERROR_MESSAGE_WRONG_WIDTH);
		ArgumentChecks.isTrue(height >= MINIMUM_HEIGHT, ERROR_MESSAGE_WRONG_HEIGHT);
		ArgumentChecks.isTrue(percentage >= MINIMUM_PERCENTAGE && percentage <= MAXIMUM_PERCENTAGE,
				ERROR_MESSAGE_WRONG_PERCENTAGE);
	...
	public void flag(int x, int y) throws GameException, UserInteractionException {
		...
		if (this.getNumberOfFlagsLeft() == 0)
			throw new GameException("There are no more flags left");
