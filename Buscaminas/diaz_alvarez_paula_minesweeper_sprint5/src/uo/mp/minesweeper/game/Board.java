...
public class Board {
	...
	/**
	 * It's the minimum value for the width of the board
	 */
	public final static int MINIMUM_WIDTH = 9;

	/**
	 * It's the maximum value for the width of the board
	 */
	public final static int MAXIMUM_WIDTH = 30;
	/**
	 * It's the minimum value for the height of the board
	 */
	public final static int MINIMUM_HEIGHT = 9;
	/**
	 * It's the maximum value for the height of the board
	 */
	public final static int MAXIMUM_HEIGHT = 30;
	/**
	 * It's the error message shown if the width introduced is not between the
	 * minimum and the maximum
	 */
	public final static String ERROR_MESSAGE_WRONG_WIDTH = String.format("The width of the board must be [%s, %s]",
			MINIMUM_WIDTH, MAXIMUM_WIDTH);

	/**
	 * It's the error message shown if the height introduced is not between the
	 * minimum and the maximum
	 */
	public final static String ERROR_MESSAGE_WRONG_HEIGHT = String.format("The height of the board must be [%s, %s]",
			MINIMUM_HEIGHT, MAXIMUM_HEIGHT);

	/**
	 * It's the error message shown if there are no more flags left
	 */
	private static final String ERROR_MESSAGE_NO_MORE_FLAGS_LEFT = "There are no more flags left";

	/**
	 * It's the minimum value for the percentage of mines
	 */
	public final static int MINIMUM_PERCENTAGE = 0;
	/**
	 * It's the maximum value for the percentage of mines
	 */
	public final static int MAXIMUM_PERCENTAGE = 100;
	 ...
	 * 
	 * @param width      is the width of the board. It must be a value between
	 *                   MAXIMUM_WIDTH and MINIMUM_WIDTH.
	 * @param height     is the height of the board. It must be a value between
	 *                   MAXIMUM_HEIGHT and MINIMUM_HEIGHT
	 * @param percentage is the percentage of mines in the board. It must be greater
	 *                   than or equal to MINIMUM_PERCENTAGE and less than or equal
	 *                   to MAXIMUM_PERCENTAGE
	 * @throws IllegalArgumentException if the percentage is not valid
	 * @throws GameException            if the width or the height is not valid
	 */
	public Board(int width, int height, int percentage) throws GameException {
		if (width < MINIMUM_WIDTH || width > MAXIMUM_WIDTH)
			throw new GameException(ERROR_MESSAGE_WRONG_WIDTH);

		if (height < MINIMUM_HEIGHT || height > MAXIMUM_HEIGHT)
			throw new GameException(ERROR_MESSAGE_WRONG_HEIGHT);

		ArgumentChecks.isTrue(percentage >= MINIMUM_PERCENTAGE && percentage <= MAXIMUM_PERCENTAGE,
				ERROR_MESSAGE_WRONG_PERCENTAGE);
	...
	protected Board(int mines, Square[][] squares) {
		...
		int MAXIMUM_NUMBER_OF_MINES = squares[0].length * squares.length;
		ArgumentChecks.isTrue(mines >= MINIMUM_PERCENTAGE && mines <= MAXIMUM_NUMBER_OF_MINES,
	...
	/**
	 * Checks if the x-coordinate and the y-coordinate are correct.
	 * 
	 * @param x is the x-coordinate of the square in the board. It must be in
	 *          [GameMove.MINIMUM_COORDINATE,width)
	 * @param y is the y-coordinate of the square in the board. It must be in
	 *          [GameMove.MINIMUM_COORDINATE, height)
	 * @throws UserInteractionException if one of the coordinates is not correct
	 */
	private void checkCoordinates(int x, int y) throws UserInteractionException {
		try {
			ArgumentChecks.isTrue(x >= GameMove.MINIMUM_COORDINATE && x < getNumberOfColumns(),
					ERROR_MESSAGE_WRONG_X_COORDINATE);
			ArgumentChecks.isTrue(y >= GameMove.MINIMUM_COORDINATE && y < getNumberOfRows(),
					ERROR_MESSAGE_WRONG_Y_COORDINATE);
		} catch (IllegalArgumentException exception) {
			throw new UserInteractionException(exception.getMessage());
		}
	}
	 ...
	 *                   and non zero integer. It must be greater than or equal to
	 *                   MINIMUM_PERCENTAGE and less than or equal to
	 *                   MAXIMUM_PERCENTAGE
	 * @throws IllegalArgumentException if one of the parameters is not valid
	 */
	private void calculateNumberOfMines(int width, int height, int percentage) {
		ArgumentChecks.isTrue(width >= MINIMUM_WIDTH && width <= MAXIMUM_WIDTH, ERROR_MESSAGE_WRONG_WIDTH);
		ArgumentChecks.isTrue(height >= MINIMUM_HEIGHT && height <= MAXIMUM_HEIGHT, ERROR_MESSAGE_WRONG_HEIGHT);
		ArgumentChecks.isTrue(percentage >= MINIMUM_PERCENTAGE && percentage <= MAXIMUM_PERCENTAGE,
				ERROR_MESSAGE_WRONG_PERCENTAGE);
	...
	/**
	 * Sets the field numberOfMines as the given number of mines if it's the real
	 ...
	 * @throws GameException            if the square wasn't closed
	 * @throws UserInteractionException if any of the parameters is wrong
	 *                                  (checkCoordinates(x,y))
	 */
	public boolean stepOn(int x, int y) throws GameException, UserInteractionException {
		checkCoordinates(x, y);
	 ...
	 *         and doesn't have a mine
	 * @throws IllegalArgumentException if the square is null
	 */
	 ...
	 * @param x is the x-coordinate of the square in the board
	 * @param y is the y-coordinate of the square in the board
	 * @throws GameException            if the box was already flagged or there no
	 *                                  more flags left
	 * @throws UserInteractionException if any of the parameters is wrong
	 *                                  (checkCoordinates(x,y))
	 */
	public void flag(int x, int y) throws GameException, UserInteractionException {
		checkCoordinates(x, y);
		if (this.getNumberOfFlagsLeft() == 0)
			throw new GameException(ERROR_MESSAGE_NO_MORE_FLAGS_LEFT);
	 ...
	 *                                  (checkCoordinates(x,y))
	 */
	public void unflag(int x, int y) throws GameException, UserInteractionException {
		checkCoordinates(x, y);
	...
	/**
	 * @param i is the y coordinate
	 * @param j is the x coordinate
	 * @return if a square is in an edge of the board
	 */
	private boolean isAnEdge(int i, int j) {
		int width = getNumberOfColumns();
		int height = getNumberOfRows();

		return (i == 0 && j == 0) || (i == 0 && j == width - 1) || (i == height - 1 && j == width - 1)
				|| (i == height - 1 && j == 0);
	}
}
