...
public class Board {
	 ...
	 */
	public final static String ERROR_MESSAGE_NULL_SQUARE = "The board must have the number of mines indicated";
	 ...
	 * the board, it sets all the actions if the squares
	 * 
	...
	public boolean stepOn(int x, int y) {
		...
		// List<Square> neighbours = getNeighbours(this.squares[y][x], x, y);
		// setAction(this.squares[y][x], neighbours);
	 ...
	 *                   doesn't have a mine and are closed
	 * @throws IllegalArgumentExpression if any of the parameters is wrong
	 */
	private void setAction(Square square, List<Square> neighbours) {
		...
		if (square.getValue() == Square.EMPTY_SQUARE) {
			square.setAction(new ClearAction(neighbours));

		} else if (square.getValue() == Square.SQUARE_WITH_MINE) {
	 ...
	 * 
	 * @param square is the given square to get the neighbours
	 * @param x      is the x-coordinate of the square in the board
	 * @param y      is the y-coordinate of the square in the board
		...
		for (int i = y - 1; i <= y + 1; i++) {
			...
			for (int j = x - 1; j <= x + 1; j++) {
				...
				if (this.squares[i][j].isClosed() && !this.squares[i][j].hasMine()) {
					...
					// setAction(this.squares[i][j], j, i);
				}
			}
		}
		return neighbours;
	...
	/**
	 * Sets all the actions of all the squares in the board
	 */
	...
	/**
	 * Prints the state of board. Each row ends with "[" and end with "]". Each
