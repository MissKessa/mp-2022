...
public class ConsoleGameInteractor implements GameInteractor {
	...
	public GameMove askMove(int rows, int columns) throws UserInteractionException {
		ArgumentChecks.isTrue(rows >= GameMove.MINIMUM_COORDINATE, ERROR_MESSAGE_NEGATIVE_MAX_ROW);
		ArgumentChecks.isTrue(columns >= GameMove.MINIMUM_COORDINATE, ERROR_MESSAGE_NEGATIVE_MAX_COLUMN);
...
//		}
	...
	/**
	 * Asks for the a coordinate (It must be greater than or equal to 0 and less
	 * than or equal to the upperLimit)
	 * 
	...
	private int askCoordinate(String message, int upperLimit) throws UserInteractionException {
		...
		try {
			...
			throw new UserInteractionException("The coordinate must be a number");
		}
		if (!(coordinate >= GameMove.MINIMUM_COORDINATE && coordinate <= upperLimit)) {
			throw new UserInteractionException(
					String.format("The coordinate must be in [%s,%s]", GameMove.MINIMUM_COORDINATE, upperLimit));
		}
		return coordinate;
	}
	...
	private char askOperation(String message) throws UserInteractionException {
		...
		try {
			...
			throw new UserInteractionException("The operation must be a character");
		}

		if (!(operation == GameMove.STEP_ON_CHARACTER || operation == GameMove.FLAG_CHARACTER
				|| operation == GameMove.UNFLAG_CHARACTER)) {
			throw new UserInteractionException(String.format("The operation must be '%s', '%s' or '%s'",
					GameMove.STEP_ON_CHARACTER, GameMove.FLAG_CHARACTER, GameMove.UNFLAG_CHARACTER));
		}
		return operation;
	}
	 ...
	 *                greater than or equal to GameMove.MINIMUM_COORDINATE
	 * @throws IllegalArgumentException if the parameter is invalid
	 */
	 ...
	 *             or equal to GameMove.MINIMUM_COORDINATE
	 * @throws IllegalArgumentException if the parameter is invalid
	 */
